...
pub fn sys_mmap(args: &SyscallArgs) -> SyscallResult {
    ...
    let vaddr = if flags & MAP_FIXED != 0 {
        VirtualAddr::new(addr_hint as usize)
    } else {
        ...
        *next = VirtualAddr::new(base);
        VirtualAddr::new(base)
    };

    // Allocate and map pages; on failure, unmap and free everything allocated so far
    for i in 0..pages {
...
pub fn sys_brk(args: &SyscallArgs) -> SyscallResult {
    ...
    if new_end > old_end {
        ...
        while vaddr < new_end {
            let paddr = match crate::PAGE_ALLOCATOR.alloc(1) {
                Ok(p) => p,
                Err(_) => return current.as_u64() as SyscallResult,
            };
            zero_page(paddr);
